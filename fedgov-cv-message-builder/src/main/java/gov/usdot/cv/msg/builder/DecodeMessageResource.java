...
import gov.usdot.cv.msg.builder.util.DecodeMessageException;
import gov.usdot.cv.msg.builder.util.DecodeMessageResult;
		...
		try {
			...
			try {
				...
				try {
					...
					returnStatusObject.put("decodedMessage",
							"Error parsing input message text to bytes.  Error was: " + e.getMessage());
